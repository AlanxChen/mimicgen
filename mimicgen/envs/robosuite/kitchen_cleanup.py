...
class KitchenCleanupEnv(SingleArmEnv):
    """
    Kitchen cleanup environment where the robot places the bread object into
    the pot and closes the drawer.
    """
    def __init__(
        self,
        robots,
    ...
    def reward(self, action=None):
        ...
        The reward is based on the distance between the cube and the target position.
        """
        reward = 0.
    ...
    def _load_model(self):
        ...
        ingredient_size = [0.015, 0.020, 0.02]
        ...
        if not self.deterministic_reset:
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                if obj is self.cabinet_object:
                    ...
                    # joint_id = self.sim.model.joint_name2id("CabinetObject_goal_slidey")
                    # self.sim.data.qpos[joint_id]=0
                    ...
                    print(self.sim.data.qpos[joint_addr])
                    self.sim.data.qpos[joint_addr] = -0.07
                    # breakpoint()
                    # print(self.sim.data.qpos[drawer_joint_id])
                    # breakpoint()
                else:
    ...
    def _check_success(self):
        """
        Checks if the cube is placed at the target position.
        """
        ...
        # Check if the cube is within a small distance of the target position
        # breakpoint()
        return object_in_pot and cabinet_closed
...
class Kitchen_Cleanup_T0(KitchenCleanupEnv):
    """Paper-name alias for the current kitchen cleanup task variant."""
    pass
