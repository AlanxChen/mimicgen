...
from mimicgen.models.robosuite.objects import DarkDrawerObject
from robosuite_task_zoo.models.kitchen import StoveObject, ButtonObject
...
class TableCleanupEnv(SingleArmEnv_MG):
    def __init__(
        self,
        robots,
    ...
    def _load_model(self):
        ...
        xpos = (xpos[0]-0.05, xpos[1] + 0.05, xpos[2])
        ...
        self.cabinet_object = DarkDrawerObject(
            name="CabinetObject")
        ...
        stove_body = self.stove_object_1.get_obj(); stove_body.set("pos", array_to_string((-0.1, 0.23, 0.02))); mujoco_arena.table_body.append(stove_body)

        self.button_object_1 = ButtonObjectNew(
            name="Button1",
        )
        # button_body = self.button_object_1.get_obj(); button_body.set("quat", array_to_string((0.1, 0., 0., 1.)));  button_body.set("pos", array_to_string((0.2, 0.10, 0.02))); mujoco_arena.table_body.append(button_body)
        for obj_body in [
                self.cabinet_object,
        ]:
        ...
        for obj_body in [
            ...
            for material in [darkwood, metal]:
                tex_element, mat_element, _, used = add_material(root=obj_body.worldbody,
                                                                 naming_prefix=obj_body.naming_prefix,
                                                                 custom_material=deepcopy(material))
                obj_body.asset.append(tex_element)
        ...
        if not self.deterministic_reset:
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                if obj is self.cabinet_object:
                    ...
                    drawer_joint_name = "CabinetObject_goal_slidey"
                    drawer_joint_id = self.sim.model.joint_name2id(drawer_joint_name)
                    joint_addr = self.sim.model.jnt_qposadr[drawer_joint_id]
                    self.sim.data.qpos[joint_addr] = -0.12
                elif obj is self.button_object_1:
                    # object is fixture - set pose in model
                    body_id = self.sim.model.body_name2id(obj.root_body)
                    obj_pos_to_set = np.array(obj_pos)
                    obj_pos_to_set[2] =  0.905 # hardcode z-value to correspond to parent class
    ...
    def _check_success(self):
        ...
        cabinet_closed = self.sim.data.qpos[self.cabinet_qpos_addrs] > -0.015

        stove_turned_off = not self.buttons_on[1]
        # print("Object in drawer: ", object_in_drawer)

        return object_in_drawer and cabinet_closed and stove_turned_off
...
class TableCleanup_D0(TableCleanupEnv,SingleArmEnv_MG):
    def __init__(self, **kwargs):
        TableCleanupEnv.__init__(self, **kwargs)
    ...
    def _get_observations(self, force_update=False):
        ...
        """
        self._post_process()
        return TableCleanupEnv._get_observations(self, force_update=force_update)


class Table_Cleanup_T0(TableCleanup_D0):
    """Paper-name alias for the current table cleanup task variant."""
    pass
